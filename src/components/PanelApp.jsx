        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1500 }}>
            <thead><tr style={{ background: '#231F20' }}>
              ...
              <th style={{ ...th, color: '#D4D2C6', background: '#fee2e2' }}>0%</th>
              <th style={{ ...th, color: '#D4D2C6', background: '#fef3c7' }}>80%</th>
              <th style={{ ...th, color: '#D4D2C6', background: '#dcfce7' }}>100%</th>
              <th style={{ ...th, color: '#D4D2C6', background: '#dbeafe' }}>120%</th>
              <th style={{ ...th, color: '#D4D2C6' }}>Coment. Lider</th>
            ...
            <tbody>{objetivos.map(function(obj) { return (
              <tr key={obj.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                ...
                <td style={{ ...td, background: '#fee2e2', fontSize: 10, maxWidth: 130 }}>
                  {obj.alcance_0_descripcion && <div style={{ color: '#dc2626' }}>{obj.alcance_0_descripcion}</div>}
                  {obj.alcance_0_fecha && <div style={{ color: '#dc2626', fontSize: 9 }}>📅 {new Date(obj.alcance_0_fecha + 'T12:00:00').toLocaleDateString('es-AR')}</div>}
                  {!obj.alcance_0_descripcion && !obj.alcance_0_fecha && '-'}
                </td>
                <td style={{ ...td, background: '#fef3c7', fontSize: 10, maxWidth: 130 }}>
                  {obj.alcance_80_descripcion && <div style={{ color: '#92400e' }}>{obj.alcance_80_descripcion}</div>}
                  {obj.alcance_80_fecha && <div style={{ color: '#92400e', fontSize: 9 }}>📅 {new Date(obj.alcance_80_fecha + 'T12:00:00').toLocaleDateString('es-AR')}</div>}
                  {!obj.alcance_80_descripcion && !obj.alcance_80_fecha && '-'}
                </td>
                <td style={{ ...td, background: '#dcfce7', fontSize: 10, maxWidth: 130 }}>
                  {obj.alcance_100_descripcion && <div style={{ color: '#166534' }}>{obj.alcance_100_descripcion}</div>}
                  {obj.alcance_100_fecha && <div style={{ color: '#166534', fontSize: 9 }}>📅 {new Date(obj.alcance_100_fecha + 'T12:00:00').toLocaleDateString('es-AR')}</div>}
                  {!obj.alcance_100_descripcion && !obj.alcance_100_fecha && '-'}
                </td>
                <td style={{ ...td, background: '#dbeafe', fontSize: 10, maxWidth: 130 }}>
                  {obj.alcance_120_descripcion && <div style={{ color: '#1e40af' }}>{obj.alcance_120_descripcion}</div>}
                  {obj.alcance_120_fecha && <div style={{ color: '#1e40af', fontSize: 9 }}>📅 {new Date(obj.alcance_120_fecha + 'T12:00:00').toLocaleDateString('es-AR')}</div>}
                  {!obj.alcance_120_descripcion && !obj.alcance_120_fecha && '-'}
                </td>
...
function ObjetivosColaborador({ profile }) {
  ...
  var [nuevoObjetivo, setNuevoObjetivo] = useState({ 
    ...
    alcance_0_descripcion: '', alcance_0_fecha: '',
  ...
  async function guardarObjetivo() {
    ...
    var datos = { objetivo: nuevoObjetivo.objetivo, corporativo: nuevoObjetivo.corporativo, ponderacion: nuevoObjetivo.ponderacion,
      alcance_0_descripcion: nuevoObjetivo.alcance_0_descripcion || null, alcance_0_fecha: nuevoObjetivo.alcance_0_fecha || null,
      alcance_80_descripcion: nuevoObjetivo.alcance_80_descripcion || null, alcance_80_fecha: nuevoObjetivo.alcance_80_fecha || null,
      alcance_100_descripcion: nuevoObjetivo.alcance_100_descripcion || null, alcance_100_fecha: nuevoObjetivo.alcance_100_fecha || null,
      alcance_120_descripcion: nuevoObjetivo.alcance_120_descripcion || null, alcance_120_fecha: nuevoObjetivo.alcance_120_fecha || null
    };
    if (editandoId) { await supabase.from('objetivos').update({ ...datos, editado_por_colaborador: true, fecha_edicion: new Date() }).eq('id', editandoId); }
    else { await supabase.from('objetivos').insert({ ...datos, colaborador_id: profile.id, status: 'pendiente' }); }
