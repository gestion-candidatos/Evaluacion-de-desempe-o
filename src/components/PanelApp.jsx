...
function PanelAdmin({ profile }) {
  ...
  async function cargarColabs() {
    ...
    const counts = {};
    (data || []).forEach(c => {
      const s = c.seniority || 'Sin definir';
  ...
  async function toggleActivo(colaborador) {
    ...
    await supabase.from('profiles').update({ activo: nuevoEstado }).eq('id', colaborador.id);
    cargarColabs();
  }
          ...
          if (ev.puntuaciones && ev.puntuaciones.length > 0) {
            ev.puntuaciones.forEach(p => {
              rows.push({
                ...
                'Competencia': p.competencias?.nombre || '', 'Rating': p.rating, 'Comentario': p.comentario || '',
                'Estado': ev.estado, 'Calibrado': ev.rating_calibrado || '', 'Finales': ev.comentarios_finales || '',
            ...
            rows.push({
              'Tipo': ev.tipo_evaluacion === 'autoevaluacion' ? 'Autoevaluación' : 'Evaluación Líder',
              'Competencia': '', 'Rating': '', 'Comentario': '', 'Estado': ev.estado,
              'Calibrado': ev.rating_calibrado || '', 'Finales': ev.comentarios_finales || '',
              'Fecha': new Date(ev.created_at).toLocaleDateString('es-AR')
            });
          }
        });
  ...
  return (
    <div>
      ...
      {vistaActiva === 'colaboradores' && (
        <div style={{ ...s.tarjetaStat, marginTop: 20 }}>
          <h4 style={{ margin: '0 0 16px 0' }}>👥 Gestionar Colaboradores ({colaboradores.length})</h4>
              ...
              <tr key={c.id} style={{ borderBottom: '1px solid #f1f5f9', opacity: c.activo ? 1 : 0.5 }}>
                ...
                <td style={td}>
                  <button onClick={() => toggleActivo(c)} style={{
                    padding: '4px 12px', borderRadius: 12, fontSize: 11, fontWeight: 600, border: 'none', cursor: 'pointer',
                    background: c.activo ? '#dcfce7' : '#fee2e2', color: c.activo ? '#166534' : '#dc2626'
                  }}>
                ...
                <td style={td}><button onClick={async () => {
                  ...
                  const ws = XLSX.utils.json_to_sheet(rows.length > 0 ? rows : [{ 'Sin datos': 'No hay evaluaciones' }]);
                  const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Evaluaciones');
                  XLSX.writeFile(wb, `Historial_${(c.full_name || c.email).replace(/\s/g, '_')}.xlsx`);
...
function PanelCalibracion({ colaboradores }) {
  ...
  function construirComentarios(evaluacion) {
    ...
    let texto = '';
    if (evaluacion.puntuaciones) { evaluacion.puntuaciones.forEach(p => { if (p.comentario) texto += `• ${p.competencias?.nombre || 'Competencia'}: ${p.comentario}\n`; }); }
  ...
  async function enviarResumenCalibracion(d) {
    ...
    emailjs.send('service_httvcn8', 'template_ytka22b', { to_email: d.colaborador.email, to_name: d.colaborador.full_name || d.colaborador.email, promedio: d.ratingFinal, clasificacion: clasif.texto, message: `Calibración final completada.\n\n📊 Auto: ${d.promAuto || 'N/A'}\n👥 Líder: ${d.promLider || 'N/A'}\n✅ Calibrado: ${d.ratingFinal}\n\n📝 Comentarios Auto:\n${comentariosAuto}\n\n📝 Comentarios Líder:\n${comentariosLider}` }, 'Mc-YPiWB1XNBKfhOJ').catch(err => console.log('Email colaborador:', err));
    if (d.evaluacionLider?.evaluador_id) { const { data: lider } = await supabase.from('profiles').select('email, full_name').eq('id', d.evaluacionLider.evaluador_id).single(); if (lider?.email) { emailjs.send('service_httvcn8', 'template_ytka22b', { to_email: lider.email, to_name: lider.full_name || 'Líder', promedio: d.ratingFinal, clasificacion: clasif.texto, message: `Calibración final de ${d.colaborador.full_name || d.colaborador.email}.\n\n📊 Auto: ${d.promAuto || 'N/A'}\n👥 Líder: ${d.promLider || 'N/A'}\n✅ Calibrado: ${d.ratingFinal}\n\n📝 Comentarios Auto:\n${comentariosAuto}\n\n📝 Comentarios Líder:\n${comentariosLider}` }, 'Mc-YPiWB1XNBKfhOJ').catch(err => console.log('Email líder:', err)); } }
    emailjs.send('service_httvcn8', 'template_ytka22b', { to_email: 'florencia.salvaneschi@grupo-fabric.com', to_name: 'Florencia Salvaneschi', promedio: d.ratingFinal, clasificacion: clasif.texto, message: `Historial de calibración - ${d.colaborador.full_name || d.colaborador.email}\n\n📊 Auto: ${d.promAuto || 'N/A'}\n👥 Líder: ${d.promLider || 'N/A'}\n✅ Calibrado: ${d.ratingFinal}\n\n📝 Comentarios Auto:\n${comentariosAuto}\n\n📝 Comentarios Líder:\n${comentariosLider}` }, 'Mc-YPiWB1XNBKfhOJ').catch(err => console.log('Email admin:', err));
                  ...
                  return (
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 16 }}>
                        <div style={{ flex: 1 }}><h4 style={{ color: '#231F20' }}>📝 Comentarios Autoevaluación</h4>{d.autoevaluacion?.puntuaciones?.filter(p => p.comentario).map(p => <p key={p.id} style={{ color: '#475569', fontSize: 12, margin: '2px 0' }}>• <strong>{p.competencias?.nombre}:</strong> {p.comentario}</p>)}<p style={{ color: '#231F20', fontSize: 13, marginTop: 8 }}><strong>Final:</strong> {d.autoevaluacion?.comentarios_finales || '-'}</p></div>
                        <div style={{ flex: 1 }}><h4 style={{ color: '#231F20' }}>👥 Comentarios Líder</h4>{d.evaluacionLider?.puntuaciones?.filter(p => p.comentario).map(p => <p key={p.id} style={{ color: '#475569', fontSize: 12, margin: '2px 0' }}>• <strong>{p.competencias?.nombre}:</strong> {p.comentario}</p>)}<p style={{ color: '#231F20', fontSize: 13, marginTop: 8 }}><strong>Final:</strong> {d.evaluacionLider?.comentarios_finales || '-'}</p></div>
                      </div>
                    </div>
                  );
                })()}
...
function EvaluacionesAdmin() {
  ...
  async function cargarEvaluaciones() {
    const { data } = await supabase.from('evaluaciones').select('*, colaborador:colaborador_id(email, full_name, area), evaluador:evaluador_id(email, full_name), puntuaciones(*, competencias(nombre))').order('created_at', { ascending: false });
        ...
        {detalleVisible && (() => { const ev = evaluaciones.find(e => e.id === detalleVisible); if (!ev) return null; return (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: 20 }} onClick={() => setDetalleVisible(null)}>
            <div style={{ background: 'white', borderRadius: 16, padding: 32, maxWidth: 600, width: '100%', maxHeight: '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
              ...
              <h4 style={{ color: '#231F20' }}>📝 Comentarios por Competencia</h4>{ev.puntuaciones?.filter(p => p.comentario).map(p => <p key={p.id} style={{ color: '#475569', fontSize: 13, margin: '2px 0' }}>• <strong>{p.competencias?.nombre}:</strong> {p.comentario}</p>)}{(!ev.puntuaciones || ev.puntuaciones.filter(p => p.comentario).length === 0) && <p style={{ color: '#94a3b8' }}>Sin comentarios por competencia</p>}
              <h4 style={{ marginTop: 16, color: '#231F20' }}>📝 Comentarios Finales</h4><p style={{ color: '#475569' }}>{ev.comentarios_finales || 'Sin comentarios'}</p>
            </div>
          </div>
        )})()}
      </div>
...
function PanelColaboradorConEquipo({ userId, seniority, email, nombre }) {
  ...
  useEffect(() => { async function verificarEquipo() { const { data: { session } } = await supabase.auth.getSession(); if (session) { const { count } = await supabase.from('profiles').select('*', { count: 'exact', head: true }).eq('leader_id', session.user.id); setTieneEquipo((count || 0) > 0); } } verificarEquipo(); }, []);
  return (
    <div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
...
function EquipoLider() {
  ...
  async function cargarEquipo() { const { data: { session } } = await supabase.auth.getSession(); if (!session) return; const { data } = await supabase.from('profiles').select('*').eq('leader_id', session.user.id); setEquipo(data || []); if (data) { const evalsPorColaborador = {}; for (const col of data) { const { data: autoeval } = await supabase.from('evaluaciones').select('*').eq('colaborador_id', col.id).eq('tipo_evaluacion', 'autoevaluacion').maybeSingle(); const { data: evalLider } = await supabase.from('evaluaciones').select('*').eq('colaborador_id', col.id).eq('tipo_evaluacion', 'evaluacion_lider').maybeSingle(); evalsPorColaborador[col.id] = { autoevaluacion: autoeval, evaluacionLider: evalLider }; } setEvaluaciones(evalsPorColaborador); } }
...
function EvaluacionLider({ colaborador, onVolver }) {
  ...
  async function enviar() { await guardar(); await supabase.from('evaluaciones').update({ estado: 'enviado', updated_at: new Date() }).eq('id', evaluacionLider.id); setMensaje('🎉 Evaluación enviada'); setEvaluacionLider({ ...evaluacionLider, estado: 'enviado' }); setTimeout(() => setMensaje(''), 3000); }
