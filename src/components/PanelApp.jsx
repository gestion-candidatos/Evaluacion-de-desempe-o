...
export default function PanelApp() {
  ...
  const nombreRol = profile.role === 'admin_rrhh' ? 'Admin RRHH' : profile.role === 'lider' ? 'Lider' : 'Colaborador';
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <aside style={sidebarStyle.aside}>
        ...
        <nav style={sidebarStyle.nav}>
          <button onClick={() => { setMenuActivo('desempeno'); setCicloActivo(null); }} style={{ ...sidebarStyle.menuItem, background: menuActivo === 'desempeno' ? '#D4D2C6' : 'transparent', color: menuActivo === 'desempeno' ? '#231F20' : '#D4D2C6' }}>📊 Evaluacion de Desempeno</button>
      ...
      <div style={{ flex: 1, background: '#f8fafc', minHeight: '100vh' }}>
        ...
        <main style={{ padding: 24 }}>
          ...
          {menuActivo === 'objetivos_empresa' && <PlaceholderView titulo="🏢 Objetivos Corporativos" descripcion="Modulo en desarrollo." />}
        </main>
      </div>
    </div>
  );
...
function CiclosLista({ esAdmin, onSelectCiclo, profile }) {
  const [ciclos, setCiclos] = useState([]); const [carg, setCarg] = useState(true); const [showC, setShowC] = useState(false);
  const [nom, setNom] = useState(''); const [fIni, setFIni] = useState(''); const [fFin, setFFin] = useState('');
  const [cGestion, setCGestion] = useState(null); const [todos, setTodos] = useState([]); const [parts, setParts] = useState([]);
  ...
  useEffect(() => { cargarCiclos(); if (esAdmin) cargarColabs(); }, []);
  async function cargarCiclos() { const { data } = await supabase.from('ciclos').select('*').order('fecha_inicio', { ascending: false }); setCiclos(data || []); setCarg(false); }
  ...
  async function toggleCiclo(ciclo) { const nuevo = ciclo.estado === 'activo' ? 'cerrado' : 'activo'; await supabase.from('ciclos').update({ estado: nuevo }).eq('id', ciclo.id); cargarCiclos(); }
  async function abrirGestion(ciclo) { setCGestion(ciclo.id); const { data } = await supabase.from('ciclo_colaboradores').select('colaborador_id').eq('ciclo_id', ciclo.id); setParts((data || []).map(p => p.colaborador_id)); }
  async function togglePart(cid) { if (parts.includes(cid)) { await supabase.from('ciclo_colaboradores').delete().eq('ciclo_id', cGestion).eq('colaborador_id', cid); setParts(p => p.filter(id => id !== cid)); } else { await supabase.from('ciclo_colaboradores').insert({ ciclo_id: cGestion, colaborador_id: cid }); setParts(p => [...p, cid]); } }
  if (carg) return <p>Cargando ciclos...</p>;

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}><h2 style={{ color: '#231F20', margin: 0 }}>📊 Ciclos de Evaluacion</h2>{esAdmin && <button onClick={() => setShowC(!showC)} style={s.btnPrimario}>+ Nuevo Ciclo</button>}</div>
      {showC && <div style={{ ...s.tarjetaStat, marginBottom: 20 }}><h4>Crear Nuevo Ciclo</h4><div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 12 }}><div><label>Nombre</label><input value={nom} onChange={e => setNom(e.target.value)} placeholder="Ej: 1er Semestre 2025" style={{ padding: 8, borderRadius: 6, border: '1px solid #D4D2C6', width: 200 }} /></div><div><label>Fecha Inicio</label><input type="date" value={fIni} onChange={e => setFIni(e.target.value)} style={{ padding: 8, borderRadius: 6, border: '1px solid #D4D2C6' }} /></div><div><label>Fecha Fin</label><input type="date" value={fFin} onChange={e => setFFin(e.target.value)} style={{ padding: 8, borderRadius: 6, border: '1px solid #D4D2C6' }} /></div><button onClick={crearCiclo} style={{ ...s.btnPrimario, background: '#22c55e', alignSelf: 'flex-end' }}>Crear</button></div></div>}
      {cGestion && <div style={{ ...s.tarjetaStat, marginBottom: 20, background: '#f8fafc' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}><h4>👥 Seleccionar Participantes</h4><button onClick={() => setCGestion(null)} style={s.btnInfo}>✕</button></div><p style={{ fontSize: 13, color: '#64748b', marginBottom: 12 }}>{parts.length} colaboradores seleccionados</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 8, maxHeight: 300, overflowY: 'auto' }}>{todos.map(c => (<div key={c.id} onClick={() => togglePart(c.id)} style={{ padding: '10px 14px', borderRadius: 8, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: parts.includes(c.id) ? '#231F20' : 'white', color: parts.includes(c.id) ? '#D4D2C6' : '#231F20', border: '1px solid #D4D2C6' }}><div><strong style={{ fontSize: 13 }}>{c.full_name || c.email}</strong><p style={{ fontSize: 11, margin: 0, opacity: 0.7 }}>{c.area} · {c.seniority}</p></div><span>{parts.includes(c.id) ? '✅' : '○'}</span></div>))}</div></div>}
          ...
          {ciclos.map(ciclo => (
            <div key={ciclo.id} style={{ ...s.tarjetaStat, border: '2px solid #D4D2C6' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                <button onClick={() => onSelectCiclo(ciclo)} style={{ ...s.btnPrimario, flex: 1 }}>{ciclo.estado === 'cerrado' && !esAdmin ? '👁️ Ver' : 'Entrar'}</button>
                {esAdmin && <button onClick={() => abrirGestion(ciclo)} style={s.btnSecundario}>👥</button>}
                {esSuperAdmin && <button onClick={() => toggleCiclo(ciclo)} style={{ ...s.btnSecundario, background: ciclo.estado === 'activo' ? '#fee2e2' : '#dcfce7', color: ciclo.estado === 'activo' ? '#dc2626' : '#166534', fontWeight: 600, fontSize: 12, whiteSpace: 'nowrap' }}>{ciclo.estado === 'activo' ? '🔒 Cerrar' : '🔓 Abrir'}</button>}
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setVista('calibracion')} style={vista === 'calibracion' ? s.btnPrimario : s.btnInfo}>🎯 Calibracion</button>
        ...
        {tieneAutoevaluacion && <button onClick={() => setVista('mievaluacion')} style={vista === 'mievaluacion' ? s.btnPrimario : s.btnInfo}>📝 Mi Evaluacion</button>}
...
function PanelColaboradorConEquipo({ userId, seniority, cicloId, profile, soloLectura }) {
  ...
  return <div><div style={{ display: 'flex', gap: 12, marginBottom: 20 }}><button onClick={() => setV('autoevaluacion')} style={v === 'autoevaluacion' ? s.btnPrimario : s.btnInfo}>📝 Mi Evaluacion</button>{tieneEq && <button onClick={() => setV('equipo')} style={v === 'equipo' ? s.btnPrimario : s.btnInfo}>👥 Mi Equipo</button>}</div>{v === 'autoevaluacion' ? <PanelColaborador userId={userId} seniority={seniority} cicloId={cicloId} soloLectura={soloLectura} /> : <EquipoLider cicloId={cicloId} profile={profile} soloLectura={soloLectura} />}</div>;
}
...
function PanelCalibracion({ cicloId, colabs, onHist, soloLectura }) {
  ...
  async function cargar() {
    ...
    const { data: evs } = await supabase.from('evaluaciones')
      .select('id, colaborador_id, tipo_evaluacion, evaluador_id, rating_promedio, rating_calibrado, comentario_calibracion, puntuaciones(rating, competencia_id, comentario, competencias(nombre)), colaborador:colaborador_id(id, email, full_name, area, seniority)')
      .eq('ciclo_id', cicloId)
    ...
    setDatos(resultado); setCarg(false);
  }
  ...
  async function guardarCal(evaluacionId, rating, comentario) {
    ...
    setDatos(p => p.map(d => d.evaluacionLider?.id === evaluacionId ? { ...d, ratingFinal: rating, comentarioCalibracion: comentario } : d));
  }
  ...
  function generarPDFCompleto(d) {
    ...
    var compsInfo = {};
    (d.autoevaluacion?.puntuaciones || []).concat(d.evaluacionLider?.puntuaciones || []).forEach(function(p) {
      if (!compsInfo[p.competencia_id] && p.competencias) compsInfo[p.competencia_id] = p.competencias.nombre || 'Competencia';
    });
    ...
    if (todasComps.length > 0) {
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(6);
      pdf.text('Competencia', colComp + 1, y + 5);
      pdf.text('A', colAutoR, y + 5);
      pdf.text('Comentario Autoevaluacion', colAutoC, y + 5);
      pdf.text('L', colLiderR, y + 5);
      pdf.text('Comentario Lider', colLiderC, y + 5);
      y += 9;
      pdf.setTextColor(NEGRO);

      todasComps.forEach(function(compId, index) {
        var nombre = (compsInfo[compId] || 'Competencia').substring(0, 18);
        var autoR = String(autoPunts[compId] || '-');
        var liderR = String(liderPunts[compId] || '-');
        var autoC = autoComs[compId] || '-';
        var liderC = liderComs[compId] || '-';
        var lineasAuto = pdf.splitTextToSize(autoC, 44);
        var lineasLider = pdf.splitTextToSize(liderC, 58);
        var altura = Math.max(7, Math.max(lineasAuto.length, lineasLider.length) * 3.5);
        ...
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(6);
        pdf.text(nombre, colComp + 1, y);
        pdf.setFont('helvetica', 'normal');

        pdf.setFillColor(BEIGE);
        pdf.circle(colAutoR + 4, y - 1.5, 3.5, 'F');
        pdf.setTextColor(NEGRO);
        pdf.setFontSize(6.5);
        pdf.text(autoR, colAutoR + 2.5, y + 0.5);

        lineasAuto.forEach(function(l, i) { pdf.text(l, colAutoC, y + (i * 3.2)); });

        pdf.setFillColor(NEGRO);
        pdf.circle(colLiderR + 4, y - 1.5, 3.5, 'F');
        pdf.setTextColor('#FFFFFF');
        pdf.setFontSize(6.5);
        pdf.text(liderR, colLiderR + 2.5, y + 0.5);
        pdf.setTextColor(NEGRO);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(11);
    pdf.text('RESULTADO FINAL CALIBRADO', marginX + 4, y + 8);
    pdf.setFontSize(16);
    pdf.text(rf, marginX + 4, y + 18);

    if (d.comentarioCalibracion) {
      y += 28;
      pdf.setTextColor(NEGRO);
      pdf.setFontSize(8);
      pdf.text('Justificacion: ' + d.comentarioCalibracion, marginX, y);
    }
  ...
  function verPDF(d) { generarPDFCompleto(d).save('Evaluacion_' + (d.colaborador.full_name || d.colaborador.email).replace(/ /g, '_') + '.pdf'); }
                ...
                <tr key={d.colaborador.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ ...td, textAlign: 'center' }}>
                    {d.evaluacionLider && !soloLectura ? (
                      <select value={d.ratingFinal || ''} onChange={function(e) { guardarCal(d.evaluacionLider.id, parseFloat(e.target.value), d.comentarioCalibracion || ''); }} style={{ padding: '6px 10px', borderRadius: 6, border: '2px solid #D4D2C6', fontSize: 14, fontWeight: 600, background: 'white' }} disabled={false}>
...
function PanelColaborador({ userId, seniority, cicloId, soloLectura }) {
  ...
  async function guardar() { if (soloLectura) return; const { data: ev } = await supabase.from('evaluaciones').select('id').eq('colaborador_id', userId).eq('tipo_evaluacion', 'autoevaluacion').eq('ciclo_id', cicloId).single(); if (!ev) return; const vals = Object.values(ratings).filter(function(r) { return r > 0; }); const prom = vals.length > 0 ? parseFloat((vals.reduce(function(a, b) { return a + b; }, 0) / vals.length).toFixed(1)) : null; await supabase.from('evaluaciones').update({ comentarios_finales: comFin, rating_promedio: prom }).eq('id', ev.id); for (const [cid, r] of Object.entries(ratings)) { await supabase.from('puntuaciones').upsert({ evaluacion_id: ev.id, competencia_id: cid, rating: r, comentario: comentarios[cid] || '' }, { onConflict: 'evaluacion_id, competencia_id' }); } setMsg('✅ Guardado'); setTimeout(() => setMsg(''), 2500); }
