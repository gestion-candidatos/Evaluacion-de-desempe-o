...
function ObjetivosView({ profile }) {
  ...
  const tieneEquipo = true; // Los gerentes y líderes tienen equipo

  if (esGerente && tieneEquipo) {
    return <ObjetivosGerente profile={profile} />;
  }
  return <ObjetivosColaborador profile={profile} />;
}

// Vista del Gerente: Gestiona objetivos de su equipo
    ...
    <div>
      ...
      {equipo.length === 0 ? <p style={{ color: '#94a3b8' }}>No tienes colaboradores asignados.</p> : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
          {equipo.map(col => (
            <div key={col.id} onClick={() => setColaboradorSeleccionado(col)} style={{ ...s.tarjetaStat, cursor: 'pointer', border: '2px solid #D4D2C6', transition: 'all 0.2s' }}>
...
// Gestión de objetivos de un colaborador (vista gerente)
            ...
            <tbody>
              {objetivos.map(obj => (
                <tr key={obj.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <td style={td}>{obj.alcance_0_fecha ? new Date(obj.alcance_0_fecha).toLocaleDateString('es-AR') : '-'}</td>
                  <td style={td}>{obj.alcance_80_fecha ? new Date(obj.alcance_80_fecha).toLocaleDateString('es-AR') : '-'}</td>
                  <td style={td}>{obj.alcance_100_fecha ? new Date(obj.alcance_100_fecha).toLocaleDateString('es-AR') : '-'}</td>
                  <td style={td}>{obj.alcance_120_fecha ? new Date(obj.alcance_120_fecha).toLocaleDateString('es-AR') : '-'}</td>
                  <td style={td}>
...
// Vista del Colaborador: Ve sus objetivos asignados
function ObjetivosColaborador({ profile }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#64748b', marginBottom: 20 }}>Objetivos asignados por tu gerente. Acéptalos y márcalos como completados cuando los termines.</p>

      {objetivos.length === 0 ? (
        <div style={{ ...s.tarjetaStat, textAlign: 'center', padding: 60 }}>
          <p style={{ color: '#94a3b8', fontSize: 16 }}>No tienes objetivos asignados aún.</p>
          <p style={{ color: '#94a3b8', fontSize: 14 }}>Tu gerente te asignará objetivos pronto.</p>
        </div>
      ) : (
        <div style={{ overflowX: 'auto' }}>
  ...
  return (
    <div>
      ...
      <div style={{ ...s.tarjetaStat, textAlign: 'center', padding: 60 }}>
        ...
        <p style={{ color: '#94a3b8', fontSize: 14 }}>Próximamente podrás ver los objetivos corporativos de la compañía.</p>
      </div>
    </div>
  );
}
...
// =============================================
// RESTO DE COMPONENTES (Desempeno, Calibracion, etc.)
// =============================================
