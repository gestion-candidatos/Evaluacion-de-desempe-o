...
function PanelAdminConEquipo({ profile, cicloId, tieneAutoevaluacion }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}><button onClick={() => setVista('dashboard')} style={vista === 'dashboard' ? s.btnPrimario : s.btnInfo}>📊 Dashboard</button><button onClick={() => setVista('evaluaciones')} style={vista === 'evaluaciones' ? s.btnPrimario : s.btnInfo}>📋 Evaluaciones</button><button onClick={() => setVista('calibracion')} style={vista === 'calibracion' ? s.btnPrimario : s.btnInfo}>🎯 Calibración</button><button onClick={() => setVista('feedback')} style={vista === 'feedback' ? s.btnPrimario : s.btnInfo}>💬 Feedback</button><button onClick={() => setVista('equipo')} style={vista === 'equipo' ? s.btnPrimario : s.btnInfo}>👥 Mi Equipo</button>{tieneAutoevaluacion && <button onClick={() => setVista('mievaluacion')} style={vista === 'mievaluacion' ? s.btnPrimario : s.btnInfo}>📝 Mi Evaluación</button>}<button onClick={() => setVista('colaboradores')} style={vista === 'colaboradores' ? s.btnPrimario : s.btnInfo}>👥 Participantes</button></div>
...
// =============================================
// DETALLE DE AUTOEVALUACIÓN (CORREGIDO)
// =============================================
    ...
    <div style={{ marginTop: 16, background: 'white', borderRadius: 12, border: '2px solid #D4D2C6', overflow: 'hidden' }}>
      ...
      <div style={{ padding: 20 }}>
        ...
        {puntuaciones.length === 0 ? (
          <p style={{ color: '#94a3b8', textAlign: 'center', padding: 20 }}>No hay competencias calificadas aún.</p>
        ) : (
                ...
                {puntuaciones.map((p, index) => (
                  <tr key={p.id || index} style={{ background: index % 2 === 0 ? 'white' : '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                    <td style={{ padding: '12px 16px', fontSize: 14, color: '#231F20', fontWeight: 500 }}>
                      {p.competencias?.nombre || `ID: ${p.competencia_id}`}
                    </td>
                    <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: 10, background: '#231F20', color: '#D4D2C6', fontSize: 16, fontWeight: 700 }}>{p.rating}</span>
                    </td>
...
// =============================================
// EQUIPO LIDER (CORREGIDO - Carga nombres de competencias)
// =============================================
function EquipoLider({ cicloId, profile }) {
  ...
  async function cargar() {
    ...
    // Para cada miembro, cargar su autoevaluación CON nombres de competencias
    const eq = await Promise.all(d.map(async (c) => {
      // Primero obtener la evaluación
      const { data: ae } = await supabase.from('evaluaciones').select('id, estado, rating_promedio, comentarios_finales').eq('colaborador_id', c.id).eq('tipo_evaluacion', 'autoevaluacion').eq('ciclo_id', cicloId).maybeSingle();
      
      let puntuacionesConNombre = [];
      if (ae) {
        // Luego obtener las puntuaciones con JOIN a competencias
        const { data: punts } = await supabase.from('puntuaciones').select('id, rating, comentario, competencia_id, competencias!inner(nombre)').eq('evaluacion_id', ae.id);
        puntuacionesConNombre = punts || [];
      }
      ...
      return { 
        ...c, 
        autoevaluacion: ae ? { ...ae, puntuaciones: puntuacionesConNombre } : null, 
        evaluacionLider: le, 
        feedback: fb 
      };
    }));
