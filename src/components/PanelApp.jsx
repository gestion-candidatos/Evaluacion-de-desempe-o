...
function DashboardView({ stats, colabs }) {
  return (
    <div>
      <div style={s.grid}>
        <div style={s.tarjetaStat}><p>👥 Participantes</p><p style={{ fontSize: 36, fontWeight: 700, color: '#231F20' }}>{colabs.length}</p></div>
...
function EvaluacionesAdmin({ cicloId }) {
  ...
  useEffect(function() {
    (async function() {
      const { data } = await supabase.from('evaluaciones').select('id,colaborador_id,tipo_evaluacion,estado,rating_promedio,rating_calibrado,created_at,colaborador:colaborador_id(email,full_name)').eq('ciclo_id', cicloId).order('created_at', { ascending: false });
      setEvs(data || []);
      setCarg(false);
    })();
  }, [cicloId]);
