...
function PanelAdminConEquipo({ profile, cicloId, tieneAutoevaluacion }) {
  ...
  // NUEVOS ESTADOS para gráficos del dashboard
  const [distribucionRatings, setDistribucionRatings] = useState([]);
  const [promedioPorCompetencia, setPromedioPorCompetencia] = useState([]);

  useEffect(() => { cargar(); }, [cicloId]);

  async function cargar() {
    const [{ count: t }, { count: e }, { data: p }, { data: f }, { data: puntuacionesData }] = await Promise.all([
      ...
      // Obtener TODAS las puntuaciones del ciclo con nombres de competencia
      supabase.from('puntuaciones').select('rating, competencia_id, competencias!inner(nombre)').eq('evaluacion_id.ciclo_id', cicloId).order('competencia_id')
    ]);
    ...
    if (puntuacionesData && puntuacionesData.length > 0) {
      const distribucion = {};
      puntuacionesData.forEach(p => {
        const r = Math.round(p.rating); // Redondear a entero
        distribucion[r] = (distribucion[r] || 0) + 1;
      });
      const distArray = [1, 2, 3, 4, 5].map(rating => ({
        rating,
        cantidad: distribucion[rating] || 0,
        porcentaje: puntuacionesData.length > 0 ? Math.round(((distribucion[rating] || 0) / puntuacionesData.length) * 100) : 0
      }));
      setDistribucionRatings(distArray);

      // Calcular promedio por competencia
      const competenciaMap = {};
      puntuacionesData.forEach(p => {
        if (!p.competencias) return;
        const nombre = p.competencias.nombre;
        if (!competenciaMap[nombre]) competenciaMap[nombre] = { nombre, suma: 0, cantidad: 0 };
        competenciaMap[nombre].suma += p.rating;
        competenciaMap[nombre].cantidad += 1;
      });
      const promArray = Object.values(competenciaMap).map(c => ({
        nombre: c.nombre,
        promedio: (c.suma / c.cantidad).toFixed(1),
        cantidad: c.cantidad
      }));
      setPromedioPorCompetencia(promArray);
    }
  }
    ...
    <div>
      ...
      {vista === 'dashboard' && (
        <DashboardCompleto 
          ...
          promedioPorCompetencia={promedioPorCompetencia}
        />
      )}
...
// =============================================
// NUEVO DASHBOARD COMPLETO CON GRÁFICOS
// =============================================
function DashboardCompleto({ stats, colabs, distribucionRatings, promedioPorCompetencia }) {
  const pct = stats.total > 0 ? Math.round((stats.enviadas / stats.total) * 100) : 0;
  const coloresRatings = ['#dc2626', '#f59e0b', '#3b82f6', '#22c55e', '#8b5cf6'];
  const etiquetasRatings = ['No adecuado', 'Por debajo', 'Cumple expectativas', 'Excede expectativas', 'Distinguido'];
  const totalRatings = distribucionRatings.reduce((sum, d) => sum + d.cantidad, 0);
    ...
    <div>
      ...
      <div style={s.grid}>
        <div style={s.tarjetaStat}>
          <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>👥 Participantes</p>
          <p style={{ fontSize: 36, fontWeight: 700, color: '#231F20', margin: '8px 0' }}>{colabs.length}</p>
        </div>
        <div style={s.tarjetaStat}>
          <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>📋 Evaluaciones</p>
          <p style={{ fontSize: 36, fontWeight: 700, color: '#231F20', margin: '8px 0' }}>{stats.total}</p>
        </div>
        <div style={{ ...s.tarjetaStat, borderTop: '4px solid #231F20' }}>
          <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>✅ Completadas</p>
          <p style={{ fontSize: 36, fontWeight: 700, color: '#231F20', margin: '8px 0' }}>{stats.enviadas}</p>
        </div>
        <div style={{ ...s.tarjetaStat, borderTop: '4px solid #D4D2C6' }}>
          <p style={{ color: '#64748b', fontSize: 14, margin: 0 }}>⏳ Pendientes</p>
          <p style={{ fontSize: 36, fontWeight: 700, color: '#231F20', margin: '8px 0' }}>{stats.pendientes}</p>
        </div>
      </div>
      ...
      <div style={{ ...s.tarjetaStat, marginTop: 20 }}>
        <h4 style={{ margin: '0 0 16px 0', color: '#231F20' }}>📊 Distribución de Ratings Finales</h4>
        <p style={{ fontSize: 13, color: '#64748b', marginBottom: 16 }}>
          Densidad de calificaciones sobre un total de <strong>{totalRatings}</strong> puntuaciones
        </p>
        
        {distribucionRatings.length === 0 ? (
          <p style={{ color: '#94a3b8', textAlign: 'center', padding: 20 }}>Sin datos de evaluaciones aún.</p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {distribucionRatings.map((d, index) => (
              <div key={d.rating}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ 
                      width: 32, height: 32, borderRadius: 8, 
                      background: coloresRatings[index], color: 'white',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontWeight: 700, fontSize: 16
                    }}>
                      {d.rating}
                    </span>
                    <span style={{ fontSize: 13, color: '#475569', fontWeight: 500 }}>{etiquetasRatings[index]}</span>
                  </div>
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#231F20' }}>
                    {d.cantidad} <span style={{ fontSize: 12, color: '#64748b', fontWeight: 400 }}>({d.porcentaje}%)</span>
                  </span>
                </div>
                <div style={{ background: '#e2e8f0', borderRadius: 8, height: 20, overflow: 'hidden' }}>
                  <div style={{
                    width: `${totalRatings > 0 ? (d.cantidad / totalRatings) * 100 : 0}%`,
                    height: '100%',
                    background: coloresRatings[index],
                    borderRadius: 8,
                    transition: 'width 0.8s',
                    minWidth: d.cantidad > 0 ? '30px' : '0'
                  }} />
                </div>
              </div>
            ))}
      ...
      <div style={{ ...s.tarjetaStat, marginTop: 20 }}>
        <h4 style={{ margin: '0 0 16px 0', color: '#231F20' }}>📈 Promedio por Competencia</h4>
        <p style={{ fontSize: 13, color: '#64748b', marginBottom: 16 }}>
          Rating promedio de todas las evaluaciones del ciclo
        </p>

        {promedioPorCompetencia.length === 0 ? (
          <p style={{ color: '#94a3b8', textAlign: 'center', padding: 20 }}>Sin datos de evaluaciones aún.</p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {promedioPorCompetencia.map((comp, index) => {
              const promNum = parseFloat(comp.promedio);
              let colorBarra = '#3b82f6';
              if (promNum <= 2.4) colorBarra = '#dc2626';
              else if (promNum <= 3.4) colorBarra = '#f59e0b';
              else if (promNum <= 4.4) colorBarra = '#22c55e';
              else colorBarra = '#8b5cf6';
              
              return (
                <div key={comp.nombre}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontSize: 13, color: '#475569', fontWeight: 500 }}>{comp.nombre}</span>
                    <span style={{ fontSize: 14, fontWeight: 700, color: colorBarra }}>
                      {comp.promedio}
                      <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 400, marginLeft: 4 }}>({comp.cantidad} eval.)</span>
                    </span>
                  </div>
                  <div style={{ background: '#e2e8f0', borderRadius: 8, height: 20, overflow: 'hidden' }}>
                    <div style={{
...
function EvaluacionLider({ colaborador, cicloId, onVolver }) {
  ...
  async function enviar() { await guardar(); const { data: ev } = await supabase.from('evaluaciones').select('id').eq('colaborador_id', colaborador.id).eq('tipo_evaluacion', 'evaluacion_lider').eq('ciclo_id', cicloId).single(); if (ev) await supabase.from('evaluaciones').update({ estado: 'enviado' }).eq('id', ev.id); setMsg('🎉 Enviada'); }
...
function PanelColaborador({ userId, seniority, cicloId }) {
  ...
  async function enviar() { await guardar(); const { data: ev } = await supabase.from('evaluaciones').select('id').eq('colaborador_id', userId).eq('tipo_evaluacion', 'autoevaluacion').eq('ciclo_id', cicloId).single(); if (ev) await supabase.from('evaluaciones').update({ estado: 'enviado' }).eq('id', ev.id); setMsg('🎉 Enviada'); }
