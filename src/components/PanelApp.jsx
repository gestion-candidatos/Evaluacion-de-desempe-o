  ...
  return React.createElement('div', null,
    ...
    objetivos.length === 0 ? React.createElement('p', { style: { color: '#94a3b8', textAlign: 'center', padding: 40 } }, 'Sin objetivos asignados.') :
      React.createElement('div', { style: { overflowX: 'auto' } },
        React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse', minWidth: 1400 } },
          React.createElement('thead', null,
            React.createElement('tr', { style: { background: '#231F20' } },
              React.createElement('th', { style: { ...th, color: '#D4D2C6' } }, 'Objetivo'),
              React.createElement('th', { style: { ...th, color: '#D4D2C6' } }, 'Corporativo'),
              React.createElement('th', { style: { ...th, color: '#D4D2C6' } }, 'Area'),
          ...
          React.createElement('tbody', null,
            objetivos.map(function(obj) {
              return React.createElement('tr', { key: obj.id, style: { borderBottom: '1px solid #e2e8f0' } },
                ...
                React.createElement('td', { style: td }, colaborador.area || '-'),
                ...
                React.createElement('td', { style: td },
                  obj.status === 'completado' ? React.createElement('button', { onClick: function() { validarObjetivo(obj.id); }, style: { ...s.btnPrimario, background: '#22c55e', fontSize: 12, padding: '6px 12px' } }, '✅ Validar') : null
                )
              );
            })
          )
...
function ObjetivosColaborador({ profile }) {
  ...
  return React.createElement('div', null,
    ...
    objetivos.length === 0 ?
      ...
      React.createElement('div', { style: { overflowX: 'auto' } },
        React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse', minWidth: 1200 } },
          React.createElement('thead', null,
            React.createElement('tr', { style: { background: '#231F20' } },
              React.createElement('th', { style: { ...th, color: '#D4D2C6' } }, 'Objetivo'),
              React.createElement('th', { style: { ...th, color: '#D4D2C6' } }, 'Corporativo'),
            ...
            objetivos.map(function(obj) {
              return React.createElement('tr', { key: obj.id, style: { borderBottom: '1px solid #e2e8f0' } },
                ...
                React.createElement('td', { style: td },
                  ...
                  obj.status === 'aceptado' ? React.createElement('button', { onClick: function() { completarObjetivo(obj.id); }, style: { ...s.btnPrimario, background: '#22c55e', fontSize: 12, padding: '6px 12px' } }, '✔️ Completar') : null
                )
              );
            })
          )
...
function CiclosLista({ esAdmin, onSelectCiclo }) {
  var ciclos = []; var setCiclos;
  var carg = true; var setCarg;
  var showC = false; var setShowC;
  var nom = ''; var setNom;
  var fIni = ''; var setFIni;
  var fFin = ''; var setFFin;
  var cSel = null; var setCSel;
  var todos = []; var setTodos;
  var parts = []; var setParts;
  var s1 = useState([]); ciclos = s1[0]; setCiclos = s1[1];
  var s2 = useState(true); carg = s2[0]; setCarg = s2[1];
  var s3 = useState(false); showC = s3[0]; setShowC = s3[1];
  var s4 = useState(''); nom = s4[0]; setNom = s4[1];
  var s5 = useState(''); fIni = s5[0]; setFIni = s5[1];
  var s6 = useState(''); fFin = s6[0]; setFFin = s6[1];
  var s7 = useState(null); cSel = s7[0]; setCSel = s7[1];
  var s8 = useState([]); todos = s8[0]; setTodos = s8[1];
  var s9 = useState([]); parts = s9[0]; setParts = s9[1];
  ...
  return React.createElement('div', null,
    React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 20 } },
      React.createElement('h2', null, '📊 Ciclos de Evaluacion'),
      esAdmin ? React.createElement('button', { onClick: function() { setShowC(!showC); }, style: s.btnPrimario }, '+ Nuevo Ciclo') : null
    ),
    showC ? React.createElement('div', { style: { ...s.tarjetaStat, marginBottom: 20 } },
    ...
    cSel ? React.createElement('div', { style: { ...s.tarjetaStat, marginBottom: 20, background: '#f8fafc' } },
      ...
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 8, maxHeight: 300, overflowY: 'auto' } },
        todos.map(function(c) {
          return React.createElement('div', { key: c.id, onClick: function() { toggle(c.id); }, style: { padding: 10, borderRadius: 8, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', background: parts.includes(c.id) ? '#231F20' : 'white', color: parts.includes(c.id) ? '#D4D2C6' : '#231F20', border: '1px solid #D4D2C6' } },
            React.createElement('div', null,
              React.createElement('strong', null, c.full_name || c.email),
              React.createElement('p', { style: { fontSize: 11, margin: 0, opacity: 0.7 } }, (c.area || '') + '·' + (c.seniority || ''))
            ),
    ...
    ciclos.length === 0 ? React.createElement('div', { style: { ...s.tarjetaStat, textAlign: 'center', padding: 40 } }, React.createElement('p', null, 'No hay ciclos.')) :
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 } },
        ciclos.map(function(c) {
          return React.createElement('div', { key: c.id, style: { ...s.tarjetaStat, border: '2px solid #D4D2C6' } },
            ...
            React.createElement('div', { style: { display: 'flex', gap: 8, marginTop: 12 } },
              React.createElement('button', { onClick: function() { onSelectCiclo(c); }, style: { ...s.btnPrimario, flex: 1 } }, 'Entrar'),
              esAdmin ? React.createElement('button', { onClick: function() { abrir(c); }, style: s.btnSecundario }, '👥') : null
            )
          );
        })
      )
...
function ParticipantesView({ colabs }) {
  return React.createElement('div', { style: s.tarjetaStat },
    React.createElement('h4', null, '👥 Participantes (' + colabs.length + ')'),
    React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse' } },
      React.createElement('thead', null,
        React.createElement('tr', null,
          React.createElement('th', { style: th }, 'Nombre'),
          React.createElement('th', { style: th }, 'Email'),
          React.createElement('th', { style: th }, 'Area'),
          React.createElement('th', { style: th }, 'Seniority')
        )
      ),
      React.createElement('tbody', null,
        colabs.map(function(c) {
          return React.createElement('tr', { key: c.id },
            React.createElement('td', { style: td }, c.full_name || '-'),
            React.createElement('td', { style: td }, c.email),
            React.createElement('td', { style: td }, c.area || '-'),
            React.createElement('td', { style: td }, c.seniority || '-')
          );
        })
      )
    )
...
function EvaluacionesAdmin({ cicloId }) {
  var evs = []; var setEvs;
  var carg = true; var setCarg;
  var s1 = useState([]); evs = s1[0]; setEvs = s1[1];
  var s2 = useState(true); carg = s2[0]; setCarg = s2[1];
  ...
  return React.createElement('div', { style: s.tarjetaStat },
    React.createElement('h4', null, '📋 Evaluaciones (' + evs.length + ')'),
    React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse' } },
      React.createElement('thead', null,
        React.createElement('tr', null,
          React.createElement('th', { style: th }, 'Colaborador'),
          React.createElement('th', { style: th }, 'Tipo'),
          React.createElement('th', { style: th }, 'Estado'),
          React.createElement('th', { style: th }, 'Rating'),
          React.createElement('th', { style: th }, 'Calibrado'),
      ...
      React.createElement('tbody', null,
        evs.map(function(ev) {
          return React.createElement('tr', { key: ev.id },
            React.createElement('td', { style: td }, (ev.colaborador ? ev.colaborador.full_name : null) || '-'),
            React.createElement('td', { style: td }, ev.tipo_evaluacion === 'autoevaluacion' ? 'Auto' : 'Lider'),
            React.createElement('td', { style: td }, ev.estado),
            React.createElement('td', { style: { ...td, fontWeight: 700 } }, ev.rating_promedio || '-'),
            React.createElement('td', { style: td }, ev.rating_calibrado || '-'),
            React.createElement('td', { style: td }, new Date(ev.created_at).toLocaleDateString('es-AR'))
          );
        })
      )
    )
...
function PanelCalibracion({ cicloId, colabs, onHist }) {
  ...
  async function cargar() {
    ...
    (resp.data || []).forEach(function(ev) {
      ...
      if (!mapa[ev.colaborador_id]) {
        mapa[ev.colaborador_id] = { colaborador: ev.colaborador, promAuto: null, promLider: null, ratingFinal: null, comentarioCalibracion: null, evaluacionLider: null };
      }
      if (ev.tipo_evaluacion === 'autoevaluacion') mapa[ev.colaborador_id].promAuto = ev.rating_promedio;
    ...
    colabs.forEach(function(c) { if (!mapa[c.id]) mapa[c.id] = { colaborador: c, promAuto: null, promLider: null, ratingFinal: null, comentarioCalibracion: null, evaluacionLider: null }; });
  ...
  async function guardarCal(evaluacionId, rating, comentario) {
    ...
    setDatos(function(p) { return p.map(function(d) { return d.evaluacionLider && d.evaluacionLider.id === evaluacionId ? { ...d, ratingFinal: rating, comentarioCalibracion: comentario } : d; }); });
  }
  ...
  function generarPDF(d) {
    var pdf = new jsPDF(); var y = 28;
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(11);
    pdf.text('EVALUACION DE DESEMPENO', 15, y); y += 7;
    pdf.setFontSize(9);
    pdf.text('Colaborador: ' + (d.colaborador.full_name || d.colaborador.email), 15, y); y += 5;
    pdf.text('Area: ' + (d.colaborador.area || '-') + '   Seniority: ' + (d.colaborador.seniority || '-'), 15, y); y += 10;
    pdf.setFontSize(12);
    pdf.text('Auto: ' + (d.promAuto || '-') + '   Lider: ' + (d.promLider || '-') + '   Calibrado: ' + (d.ratingFinal || '-'), 15, y + 10);
    if (d.comentarioCalibracion) { pdf.setFontSize(8); pdf.text('Justificacion: ' + d.comentarioCalibracion, 15, y + 18); }
    return pdf;
  }
  ...
  return React.createElement('div', { style: { ...s.tarjetaStat } },
    React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 16 } },
      React.createElement('h3', { style: { margin: 0, color: '#231F20' } }, 'Calibracion'),
      React.createElement('select', { value: filtro, onChange: function(e) { setFiltro(e.target.value); }, style: { padding: '8px 12px', borderRadius: 6, border: '2px solid #D4D2C6' } },
    ...
    React.createElement('div', { style: { overflowX: 'auto' } },
      React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse', minWidth: 1000 } },
        React.createElement('thead', null,
          React.createElement('tr', null,
            React.createElement('th', { style: th }, 'Colaborador'),
            React.createElement('th', { style: th }, 'Auto'),
            React.createElement('th', { style: th }, 'Lider'),
            React.createElement('th', { style: th }, 'GAP'),
            React.createElement('th', { style: th }, 'Calibrado'),
            React.createElement('th', { style: th }, 'Justificacion'),
            React.createElement('th', { style: th }, 'Hist'),
            React.createElement('th', { style: th }, 'PDF'),
            React.createElement('th', { style: th }, 'Enviar')
          )
        ),
        React.createElement('tbody', null,
          df.map(function(d) {
            ...
            return React.createElement('tr', { key: d.colaborador.id },
              React.createElement('td', { style: td }, React.createElement('strong', null, d.colaborador.full_name || d.colaborador.email)),
              React.createElement('td', { style: { ...td, textAlign: 'center', fontWeight: 700, color: clasificar(d.promAuto).color } }, d.promAuto || '-'),
              React.createElement('td', { style: { ...td, textAlign: 'center', fontWeight: 700, color: clasificar(d.promLider).color } }, d.promLider || '-'),
              React.createElement('td', { style: { ...td, textAlign: 'center', fontWeight: 700 } }, gap || '-'),
              React.createElement('td', { style: td },
                d.evaluacionLider ? React.createElement('select', {
                  value: d.ratingFinal || '',
                  ...
                  React.createElement('option', { value: '1' }, '1.0'), React.createElement('option', { value: '1.5' }, '1.5'),
                  React.createElement('option', { value: '2' }, '2.0'), React.createElement('option', { value: '2.5' }, '2.5'),
                  React.createElement('option', { value: '3' }, '3.0'), React.createElement('option', { value: '3.5' }, '3.5'),
                  React.createElement('option', { value: '4' }, '4.0'), React.createElement('option', { value: '4.5' }, '4.5'),
                  React.createElement('option', { value: '5' }, '5.0')
                ) : '-'
              ),
              React.createElement('td', { style: td },
                d.evaluacionLider ? React.createElement('input', {
                  value: d.comentarioCalibracion || '',
              ...
              React.createElement('td', { style: td },
                React.createElement('button', { onClick: function() { onHist(d.colaborador); }, style: { background: '#D4D2C6', color: '#231F20', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer' } }, '📋')
              ),
              React.createElement('td', { style: td },
                React.createElement('button', { onClick: function() { verPDF(d); }, style: { background: '#f59e0b', color: 'white', border: 'none', borderRadius: 6, padding: '4px 8px', fontSize: 11 } }, 'PDF')
              ),
              React.createElement('td', { style: td },
                d.ratingFinal ? React.createElement('button', { onClick: function() { enviarPDF(d); }, style: { background: '#231F20', color: '#D4D2C6', border: 'none', borderRadius: 6, padding: '4px 8px', fontSize: 11, fontWeight: 600 } }, 'Enviar') : '-'
              )
            );
          })
        )
...
function FeedbackAdmin({ cicloId }) {
  var fbs = []; var setFbs;
  var carg = true; var setCarg;
  var s1 = useState([]); fbs = s1[0]; setFbs = s1[1];
  var s2 = useState(true); carg = s2[0]; setCarg = s2[1];

  useEffect(function() {
    (async function() {
      var resp = await supabase.from('feedback').select('*, lider:lider_id(email, full_name), colaborador:colaborador_id(email, full_name)').eq('ciclo_id', cicloId).order('created_at', { ascending: false });
      setFbs(resp.data || []); setCarg(false);
    })();
  }, [cicloId]);
  ...
  return React.createElement('div', { style: s.tarjetaStat },
    React.createElement('h4', null, '💬 Feedback (' + fbs.length + ')'),
    fbs.length === 0 ? React.createElement('p', { style: { textAlign: 'center', padding: 20, color: '#94a3b8' } }, 'Sin registros.') :
      React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse' } },
        React.createElement('thead', null,
          React.createElement('tr', null,
            React.createElement('th', { style: th }, 'Lider'), React.createElement('th', { style: th }, 'Colaborador'),
            React.createElement('th', { style: th }, 'Comentario'), React.createElement('th', { style: th }, 'Fecha'),
            React.createElement('th', { style: th }, 'OK')
          )
        ),
        React.createElement('tbody', null,
          fbs.map(function(f) {
            return React.createElement('tr', { key: f.id },
              React.createElement('td', { style: td }, (f.lider ? f.lider.full_name : null) || '-'),
              React.createElement('td', { style: td }, (f.colaborador ? f.colaborador.full_name : null) || '-'),
              React.createElement('td', { style: td }, f.comentario_lider || '-'),
              React.createElement('td', { style: td }, f.fecha_feedback_lider ? new Date(f.fecha_feedback_lider).toLocaleDateString('es-AR') : '-'),
              React.createElement('td', { style: td }, f.confirmacion_colaborador ? '✅' : '⏳')
            );
          })
        )
      )
...
function HistorialAdmin({ colaborador, onVolver }) {
  var hist = []; var setHist;
  var carg = true; var setCarg;
  var s1 = useState([]); hist = s1[0]; setHist = s1[1];
  var s2 = useState(true); carg = s2[0]; setCarg = s2[1];

  useEffect(function() {
    (async function() {
      var resp = await supabase.from('evaluaciones_historicas').select('*').eq('colaborador_id', colaborador.id).order('fecha_evaluacion', { ascending: false });
      setHist(resp.data || []); setCarg(false);
    })();
  }, []);
  ...
  return React.createElement('div', null,
    React.createElement('button', { onClick: onVolver, style: { ...s.btnInfo, marginBottom: 16 } }, '← Volver'),
    React.createElement('h3', null, '📋 Historial: ' + (colaborador.full_name || colaborador.email)),
    hist.length === 0 ? React.createElement('p', { style: { padding: 40, color: '#94a3b8' } }, 'Sin historial.') :
      React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse' } },
        React.createElement('thead', null, React.createElement('tr', null, React.createElement('th', { style: th }, 'Fecha'), React.createElement('th', { style: th }, 'Rating'))),
        React.createElement('tbody', null,
          hist.map(function(h) {
            return React.createElement('tr', { key: h.id },
              React.createElement('td', { style: td }, new Date(h.fecha_evaluacion + 'T12:00:00').toLocaleDateString('es-AR')),
              React.createElement('td', { style: td }, h.rating_final || '-')
            );
          })
        )
      )
...
function EquipoLider({ cicloId, profile }) {
  var equipo = []; var setEquipo;
  var colSel = null; var setColSel;
  var fbVis = null; var setFbVis;
  var detalleVisible = null; var setDetalleVisible;
  var s1 = useState([]); equipo = s1[0]; setEquipo = s1[1];
  var s2 = useState(null); colSel = s2[0]; setColSel = s2[1];
  var s3 = useState(null); fbVis = s3[0]; setFbVis = s3[1];
  var s4 = useState(null); detalleVisible = s4[0]; setDetalleVisible = s4[1];
  ...
  return React.createElement('div', null,
    ...
    equipo.length === 0 ? React.createElement('p', null, 'No tienes colaboradores.') :
      React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 16 } },
        equipo.map(function(c) {
            ...
            React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 } },
              React.createElement('div', { style: { flex: 1 } },
                ...
                React.createElement('div', { style: { display: 'flex', gap: 16, marginTop: 8, fontSize: 12 } },
                  React.createElement('span', null, '📝 Auto: ', React.createElement('strong', { style: { color: (c.autoevaluacion ? c.autoevaluacion.estado === 'enviado' : false) ? '#22c55e' : '#f59e0b' } }, c.autoevaluacion && c.autoevaluacion.estado === 'enviado' ? '✅ Enviada' : '⏳ Pendiente')),
                  React.createElement('span', null, '👥 Mi eval: ', React.createElement('strong', { style: { color: (c.evaluacionLider ? c.evaluacionLider.estado === 'enviado' : false) ? '#22c55e' : c.evaluacionLider ? '#f59e0b' : '#94a3b8' } }, c.evaluacionLider && c.evaluacionLider.estado === 'enviado' ? '✅ Completada' : c.evaluacionLider ? '📝 Borrador' : '❌ Sin evaluar')),
                  React.createElement('span', null, '💬 FB: ', React.createElement('strong', { style: { color: (c.feedback ? c.feedback.confirmacion_colaborador : false) ? '#22c55e' : c.feedback ? '#f59e0b' : '#94a3b8' } }, c.feedback && c.feedback.confirmacion_colaborador ? '✅' : c.feedback ? '⏳' : '-'))
                )
              ),
              React.createElement('div', { style: { display: 'flex', gap: 8 } },
                c.autoevaluacion && c.autoevaluacion.estado === 'enviado' ? React.createElement('button', { onClick: function() { setDetalleVisible(detalleVisible === c.id ? null : c.id); }, style: { ...s.btnInfo, background: '#dbeafe', color: '#1e40af', fontWeight: 600 } }, detalleVisible === c.id ? '🔼 Ocultar' : '👁️ Ver Autoevaluacion') : null,
                React.createElement('button', { onClick: function() { setFbVis(c); }, style: { ...s.btnInfo, background: '#fef3c7', color: '#92400e' } }, '💬 FB'),
                React.createElement('button', { onClick: function() { setColSel(c); }, style: s.btnPrimario }, c.evaluacionLider ? '✏️ Editar' : '📝 Evaluar')
              )
            ),
            detalleVisible === c.id ? React.createElement(DetalleAutoEvaluacion, { autoevaluacion: c.autoevaluacion }) : null
          );
        })
      )
  );
...
function FeedbackForm({ feedback: col, cicloId, onVolver }) {
  var com = ''; var setCom;
  var fb = null; var setFb;
  var carg = true; var setCarg;
  var s1 = useState(''); com = s1[0]; setCom = s1[1];
  var s2 = useState(null); fb = s2[0]; setFb = s2[1];
  var s3 = useState(true); carg = s3[0]; setCarg = s3[1];

  useEffect(function() {
    (async function() {
      var resp = await supabase.auth.getSession();
      var resp2 = await supabase.from('feedback').select('*').eq('ciclo_id', cicloId).eq('colaborador_id', col.id).maybeSingle();
      if (resp2.data) { setFb(resp2.data); setCom(resp2.data.comentario_lider || ''); }
      else { await supabase.from('feedback').insert({ ciclo_id: cicloId, lider_id: resp.data.session.user.id, colaborador_id: col.id }); }
      setCarg(false);
    })();
  }, []);
  ...
  async function guardar() {
    var resp = await supabase.auth.getSession();
    await supabase.from('feedback').upsert({ ciclo_id: cicloId, lider_id: resp.data.session.user.id, colaborador_id: col.id, comentario_lider: com, fecha_feedback_lider: new Date() }, { onConflict: 'ciclo_id, colaborador_id' });
  ...
  return React.createElement('div', { style: { maxWidth: 600 } },
    React.createElement('button', { onClick: onVolver, style: { ...s.btnInfo, marginBottom: 16 } }, '← Volver'),
    React.createElement('h3', null, '💬 Feedback: ' + (col.full_name || col.email)),
    React.createElement('textarea', { value: com, onChange: function(e) { setCom(e.target.value); }, placeholder: 'Deja tu feedback...', style: { ...s.textarea, minHeight: 120, marginBottom: 12 } }),
    fb && fb.confirmacion_colaborador ? React.createElement('div', { style: { padding: 12, background: '#dcfce7', borderRadius: 8, marginBottom: 16 } }, '✅ Confirmado') : null,
    React.createElement('button', { onClick: guardar, style: s.btnPrimario }, '💾 Guardar')
  );
}

function EvaluacionLider({ colaborador, cicloId, onVolver }) {
  var competencias = []; var setComp;
  var ratings = {}; var setRatings;
  var comentarios = {}; var setComent;
  var comFin = ''; var setComFin;
  var msg = ''; var setMsg;
  var carg = true; var setCarg;
  var autoEval = null; var setAutoEval;
  var evalData = null; var setEvalData;
  var showInfo = {}; var setShowInfo;
  var s1 = useState([]); competencias = s1[0]; setComp = s1[1];
  var s2 = useState({}); ratings = s2[0]; setRatings = s2[1];
  var s3 = useState({}); comentarios = s3[0]; setComent = s3[1];
  var s4 = useState(''); comFin = s4[0]; setComFin = s4[1];
  var s5 = useState(''); msg = s5[0]; setMsg = s5[1];
  var s6 = useState(true); carg = s6[0]; setCarg = s6[1];
  var s7 = useState(null); autoEval = s7[0]; setAutoEval = s7[1];
  var s8 = useState(null); evalData = s8[0]; setEvalData = s8[1];
  var s9 = useState({}); showInfo = s9[0]; setShowInfo = s9[1];
  ...
  useEffect(function() {
    (async function() {
      ...
      setComp(resps[0].data || []);
      var resp3 = await supabase.from('evaluaciones').select('id, estado, rating_promedio, comentarios_finales').eq('colaborador_id', colaborador.id).eq('tipo_evaluacion', 'autoevaluacion').eq('ciclo_id', cicloId).maybeSingle();
      if (resp3.data) {
        var resp4 = await supabase.from('puntuaciones').select('id, rating, comentario, competencia_id, competencias!inner(nombre)').eq('evaluacion_id', resp3.data.id);
        setAutoEval({ ...resp3.data, puntuaciones: resp4.data || [] });
      }
      var resp5 = await supabase.from('evaluaciones').select('id, estado, comentarios_finales, rating_promedio').eq('colaborador_id', colaborador.id).eq('tipo_evaluacion', 'evaluacion_lider').eq('ciclo_id', cicloId).maybeSingle();
      if (resp5.data) {
        setEvalData(resp5.data); setComFin(resp5.data.comentarios_finales || '');
        var resp6 = await supabase.from('puntuaciones').select('rating, competencia_id, comentario').eq('evaluacion_id', resp5.data.id);
        var rm = {}; var cm = {};
        (resp6.data || []).forEach(function(p) { rm[p.competencia_id] = p.rating; cm[p.competencia_id] = p.comentario || ''; });
        setRatings(rm); setComent(cm);
      } else {
        await supabase.from('evaluaciones').insert({ colaborador_id: colaborador.id, evaluador_id: resps[1].data.session.user.id, tipo_evaluacion: 'evaluacion_lider', estado: 'borrador', ciclo_id: cicloId });
      }
      setCarg(false);
    })();
  }, []);
  ...
  async function guardar() {
    var falt = competencias.filter(function(c) { return !comentarios[c.id] || !comentarios[c.id].trim(); });
    if (falt.length > 0) { setMsg('❌ Completa: ' + falt.map(function(c) { return c.nombre; }).join(', ')); setTimeout(function() { setMsg(''); }, 4000); return; }
    if (!comFin || !comFin.trim()) { setMsg('❌ Comentarios finales obligatorios'); setTimeout(function() { setMsg(''); }, 4000); return; }
    var resp = await supabase.from('evaluaciones').select('id').eq('colaborador_id', colaborador.id).eq('tipo_evaluacion', 'evaluacion_lider').eq('ciclo_id', cicloId).single();
    if (!resp.data) return;
    ...
    await supabase.from('evaluaciones').update({ comentarios_finales: comFin, rating_promedio: prom }).eq('id', resp.data.id);
    for (var cid in ratings) {
      await supabase.from('puntuaciones').upsert({ evaluacion_id: resp.data.id, competencia_id: cid, rating: ratings[cid], comentario: comentarios[cid] || '' }, { onConflict: 'evaluacion_id, competencia_id' });
    }
  ...
  if (carg) return React.createElement('p', null, 'Cargando...');
  var enviada = evalData && evalData.estado === 'enviado';
  var prom = calcProm();

  return React.createElement('div', { style: { maxWidth: 900 } },
    React.createElement('button', { onClick: onVolver, style: { ...s.btnInfo, marginBottom: 16 } }, '← Volver'),
    React.createElement('h3', null, '📝 Evaluando a: ' + (colaborador.full_name || colaborador.email)),
    React.createElement('p', null, (colaborador.area || '') + ' · ' + (colaborador.seniority || '')),
    autoEval && autoEval.estado === 'enviado' ? React.createElement(DetalleAutoEvaluacion, { autoevaluacion: autoEval }) : null,
    competencias.map(function(comp) {
      return React.createElement('div', { key: comp.id, style: s.competenciaCard },
        React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' } },
          React.createElement('div', null, React.createElement('h5', null, comp.nombre), React.createElement('p', { style: { fontSize: 13, color: '#64748b' } }, comp.descripcion)),
          React.createElement('button', { onClick: function() { setShowInfo({ ...showInfo, [comp.id]: !showInfo[comp.id] }); }, style: s.btnInfo }, showInfo[comp.id] ? '🔼' : '🔽')
        ),
        showInfo[comp.id] ? React.createElement('div', { style: { ...s.ratingInfoBox, marginTop: 8 } },
          [1, 2, 3, 4, 5].map(function(r) { return React.createElement('div', { key: r, style: s.ratingInfoItem }, React.createElement('strong', null, 'Nivel ' + r + ': '), React.createElement(RatingDesc, { competenciaId: comp.id, rating: r })); })
        ) : null,
        React.createElement('div', { style: s.ratingRow },
          [1, 2, 3, 4, 5].map(function(r) {
            return React.createElement('button', { key: r, onClick: function() { if (!enviada) setRatings({...ratings, [comp.id]: r}); }, style: { ...s.ratingBtn, backgroundColor: ratings[comp.id] === r ? '#231F20' : '#f1f5f9', color: ratings[comp.id] === r ? 'white' : '#475569', cursor: enviada ? 'not-allowed' : 'pointer' }, disabled: enviada }, r);
          })
        ),
        React.createElement('textarea', { value: comentarios[comp.id] || '', onChange: function(e) { setComent({...comentarios, [comp.id]: e.target.value}); }, placeholder: 'Comentario obligatorio', style: { ...s.textareaSmall, borderColor: enviada ? '#D4D2C6' : (comentarios[comp.id] && comentarios[comp.id].trim() ? '#D4D2C6' : '#dc2626') }, disabled: enviada })
      );
    }),
    React.createElement(SeccionText, { titulo: '📝 Comentarios Finales (obligatorio)', valor: comFin, onChange: setComFin, disabled: enviada }),
    prom ? React.createElement('div', { style: { marginTop: 24, padding: 20, background: 'white', borderRadius: 12, border: '2px solid #231F20', textAlign: 'center' } }, React.createElement('p', null, 'Resultado Final'), React.createElement('p', { style: { fontSize: 48, fontWeight: 700, color: '#231F20' } }, prom)) : null,
    msg ? React.createElement('div', { style: s.mensajeToast }, msg) : null,
    !enviada ? React.createElement('div', { style: { display: 'flex', gap: 12, marginTop: 20 } }, React.createElement('button', { onClick: guardar, style: s.btnSecundario }, '💾 Guardar'), React.createElement('button', { onClick: enviar, style: s.btnPrimario }, '📤 Enviar')) : null
  );
}
...
function PanelColaborador({ userId, seniority, cicloId }) {
  ...
  var msg = ''; var setMsg;
  var carg = true; var setCarg;
  var evalLider = null; var setEvalLider;
  var feedback = null; var setFeedback;
  var evalData = null; var setEvalData;
  var showInfo = {}; var setShowInfo;
  var s1 = useState([]); competencias = s1[0]; setComp = s1[1];
  var s2 = useState({}); ratings = s2[0]; setRatings = s2[1];
  ...
  var s5 = useState(''); msg = s5[0]; setMsg = s5[1];
  var s6 = useState(true); carg = s6[0]; setCarg = s6[1];
  var s7 = useState(null); evalLider = s7[0]; setEvalLider = s7[1];
  var s8 = useState(null); feedback = s8[0]; setFeedback = s8[1];
  var s9 = useState(null); evalData = s9[0]; setEvalData = s9[1];
  var s10 = useState({}); showInfo = s10[0]; setShowInfo = s10[1];
  ...
  useEffect(function() {
    (async function() {
      ...
      setComp(resps[0].data || []); setEvalLider(resps[2].data); setFeedback(resps[3].data);
      if (resps[1].data) {
        setEvalData(resps[1].data); setComFin(resps[1].data.comentarios_finales || '');
        var resp5 = await supabase.from('puntuaciones').select('rating, competencia_id, comentario').eq('evaluacion_id', resps[1].data.id);
        var rm = {}; var cm = {};
        (resp5.data || []).forEach(function(p) { rm[p.competencia_id] = p.rating; cm[p.competencia_id] = p.comentario || ''; });
        setRatings(rm); setComent(cm);
      } else {
  ...
  async function guardar() {
    ...
    if (falt.length > 0) { setMsg('❌ Completa: ' + falt.map(function(c) { return c.nombre; }).join(', ')); setTimeout(function() { setMsg(''); }, 4000); return; }
    if (!comFin || !comFin.trim()) { setMsg('❌ Comentarios finales obligatorios'); setTimeout(function() { setMsg(''); }, 4000); return; }
    var resp = await supabase.from('evaluaciones').select('id').eq('colaborador_id', userId).eq('tipo_evaluacion', 'autoevaluacion').eq('ciclo_id', cicloId).single();
    if (!resp.data) return;
    var vals = Object.values(ratings).filter(function(r) { return r > 0; });
    var prom = vals.length > 0 ? parseFloat((vals.reduce(function(a, b) { return a + b; }, 0) / vals.length).toFixed(1)) : null;
    await supabase.from('evaluaciones').update({ comentarios_finales: comFin, rating_promedio: prom }).eq('id', resp.data.id);
  ...
  return React.createElement('div', { style: { maxWidth: 900 } },
    React.createElement('h3', null, '📝 Mi Autoevaluacion'),
    React.createElement('p', null, 'Seniority: ', React.createElement('strong', null, seniority || 'No definido')),
    React.createElement('p', null, 'Estado: ', React.createElement('strong', { style: { color: enviada ? '#231F20' : '#f59e0b' } }, enviada ? '✅ Enviada' : '📝 En progreso')),
    feedback ? React.createElement('div', { style: { padding: 16, background: feedback.confirmacion_colaborador ? '#dcfce7' : '#fef3c7', borderRadius: 10, marginBottom: 20 } },
      React.createElement('h4', null, '💬 Feedback'),
      React.createElement('p', null, feedback.comentario_lider || 'Sin comentarios.'),
      !feedback.confirmacion_colaborador ? React.createElement('button', { onClick: confirmarFB, style: { ...s.btnPrimario, background: '#22c55e', marginTop: 8, fontSize: 13 } }, '✅ Confirmar') : React.createElement('p', { style: { color: '#166534', marginTop: 8 } }, '✅ Confirmado')
    ) : null,
    evalLider && evalLider.rating_calibrado ? React.createElement('div', { style: { padding: 16, background: '#D4D2C6', borderRadius: 10, marginBottom: 20, textAlign: 'center' } },
      React.createElement('p', null, '🎯 Resultado Final Calibrado'),
      React.createElement('p', { style: { fontSize: 36, fontWeight: 700 } }, evalLider.rating_calibrado)
    ) : null,
    competencias.map(function(comp) {
      return React.createElement('div', { key: comp.id, style: s.competenciaCard },
        React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' } },
...
function RatingDesc({ competenciaId, rating }) {
  ...
  useEffect(function() {
    (async function() {
      var resp = await supabase.from('rating_descriptions').select('titulo, descripcion').eq('competencia_id', competenciaId).eq('rating', rating).single();
      if (resp.data) setDesc(resp.data.titulo + ': ' + resp.data.descripcion);
    })();
...
function SeccionText({ titulo, valor, onChange, disabled }) {
  return React.createElement('div', { style: { marginBottom: 24 } },
    React.createElement('h4', { style: s.seccionTitulo }, titulo),
    React.createElement('textarea', { value: valor, onChange: function(e) { onChange(e.target.value); }, style: { ...s.textarea, borderColor: disabled ? '#D4D2C6' : (valor && valor.trim() ? '#D4D2C6' : '#dc2626') }, disabled: disabled })
  );
}
