...
function DesempenoView({ profile, cicloActivo, setCicloActivo }) {
  ...
  return (
    <div><button onClick={() => setCicloActivo(null)} style={{ ...s.btnInfo, marginBottom: 16 }}>← Volver a Ciclos</button><h2>📊 {cicloActivo.nombre}</h2><p style={{ color: '#64748b', marginBottom: 20 }}>{new Date(cicloActivo.fecha_inicio).toLocaleDateString('es-AR')} · {cicloActivo.estado}</p>
      {esAdmin ? <PanelAdminConEquipo profile={profile} cicloId={cicloActivo.id} /> : profile.role === 'lider' ? <PanelLiderConEquipo cicloId={cicloActivo.id} profile={profile} /> : <PanelColaboradorConEquipo userId={profile.id} seniority={profile.seniority} cicloId={cicloActivo.id} profile={profile} />}
    </div>
  );
}

function PanelAdminConEquipo({ profile, cicloId }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        {!esGerente && <button onClick={()=>setVista('mievaluacion')} style={vista==='mievaluacion'?s.btnPrimario:s.btnInfo}>📝 Mi Evaluación</button>}
      ...
      {vista==='evaluaciones'&&<EvaluacionesAdmin cicloId={cicloId} onHist={setHist} />}
      ...
      {vista==='mievaluacion'&&!esGerente&&<PanelColaborador userId={profile.id} seniority={profile.seniority} cicloId={cicloId} />}
...
function PanelColaboradorConEquipo({ userId, seniority, cicloId, profile }) {
  ...
  if(verif)return<p>Verificando...</p>;if(!part)return<div style={{...s.tarjetaStat,textAlign:'center',padding:40}}><p>No estás participando en este ciclo.</p></div>;
...
function CiclosLista({ esAdmin, onSelectCiclo }) {
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',marginBottom:20}}><h2>📊 Ciclos de Evaluación</h2>{esAdmin&&<button onClick={()=>setShowC(!showC)} style={s.btnPrimario}>+ Nuevo Ciclo</button>}</div>
...
function EvaluacionesAdmin({ cicloId, onHist }) {
...
function PanelCalibracion({ cicloId, colabs, onHist }) {
  ...
  function generarPDF(d){const pdf=new jsPDF();let y=28;try{pdf.addImage('/logo.jpg','JPEG',15,8,30,15);}catch(e){}pdf.setFont('helvetica','bold');pdf.setFontSize(11);pdf.text('EVALUACIÓN DE DESEMPEÑO',15,y);y+=7;pdf.setFont('helvetica','normal');pdf.setFontSize(9);pdf.text(`Colaborador: ${d.colaborador.full_name||d.colaborador.email}`,15,y);y+=5;pdf.text(`Área: ${d.colaborador.area||'-'}   Seniority: ${d.colaborador.seniority||'-'}`,15,y);y+=10;pdf.setFontSize(12);pdf.text(`Auto: ${d.promAuto||'-'}   Líder: ${d.promLider||'-'}   Calibrado: ${d.ratingFinal||'-'}`,15,y+10);if(d.comentarioCalibracion){pdf.setFontSize(8);pdf.text(`Justificación: ${d.comentarioCalibracion}`,15,y+18);}return pdf;}
  ...
  const df = filtro==='Todas'?datos:datos.filter(d=>d.colaborador.area===filtro);

  if(carg)return<p>⏳ Cargando calibración...</p>;
  return <div style={{...s.tarjetaStat}}>
    <div style={{display:'flex',justifyContent:'space-between',marginBottom:16}}><h3>🎯 Calibración - Auto vs Líder</h3><select value={filtro} onChange={e=>setFiltro(e.target.value)} style={{padding:'8px 12px',borderRadius:6,border:'2px solid #D4D2C6'}}>{areas.map(a=><option key={a} value={a}>{a}</option>)}</select></div>
    <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',minWidth:1200}}><thead><tr><th style={th}>Colaborador</th><th style={th}>Área</th><th style={th}>Seniority</th><th style={th}>Auto</th><th style={th}>Líder</th><th style={th}>GAP</th><th style={th}>Calibrado</th><th style={th}>Justificación</th><th style={th}>Hist</th><th style={th}>PDF</th><th style={th}>Enviar</th></tr></thead>
      <tbody>{df.map(d=>{const clasA=clasificar(d.promAuto);const clasL=clasificar(d.promLider);const clasF=clasificar(d.ratingFinal);const gap=d.promAuto&&d.promLider?(parseFloat(d.promLider)-parseFloat(d.promAuto)).toFixed(1):null;return(
        <tr key={d.colaborador.id}><td style={td}><strong>{d.colaborador.full_name||d.colaborador.email}</strong></td><td style={td}>{d.colaborador.area||'-'}</td><td style={td}><span style={{padding:'3px 8px',borderRadius:12,fontSize:11,fontWeight:600,background:'#D4D2C6',color:'#231F20'}}>{d.colaborador.seniority||'-'}</span></td>
          <td style={{...td,textAlign:'center'}}>{d.promAuto?<div><span style={{fontSize:18,fontWeight:700,color:clasA.color}}>{d.promAuto}</span><div style={{fontSize:10,color:clasA.color}}>{clasA.texto}</div></div>:'-'}</td>
          <td style={{...td,textAlign:'center'}}>{d.promLider?<div><span style={{fontSize:18,fontWeight:700,color:clasL.color}}>{d.promLider}</span><div style={{fontSize:10,color:clasL.color}}>{clasL.texto}</div></div>:'-'}</td>
          ...
          <td style={td}>{d.evaluacionLider?<select value={d.ratingFinal||''} onChange={e=>guardarCal(d.evaluacionLider.id,parseFloat(e.target.value),d.comentarioCalibracion||'')} style={{padding:4,borderRadius:6,border:`2px solid ${clasF.color}`,fontWeight:600,color:clasF.color}}><option value="">-</option><option value="1">1.0</option><option value="1.5">1.5</option><option value="2">2.0</option><option value="2.5">2.5</option><option value="3">3.0</option><option value="3.5">3.5</option><option value="4">4.0</option><option value="4.5">4.5</option><option value="5">5.0</option></select>:'-'}</td>
          <td style={td}>{d.evaluacionLider?<input value={d.comentarioCalibracion||''} onChange={e=>guardarCal(d.evaluacionLider.id,d.ratingFinal||null,e.target.value)} placeholder="Justificar..." style={{width:'100%',padding:4,borderRadius:6,border:'1px solid #D4D2C6',fontSize:11}}/>:'-'}</td>
          <td style={td}><button onClick={()=>onHist(d.colaborador)} style={{background:'#D4D2C6',color:'#231F20',border:'none',borderRadius:6,padding:'4px 8px',cursor:'pointer'}}>📋</button></td>
...
function FeedbackAdmin({ cicloId }) {
  const [fbs,setFbs]=useState([]);const[carg,setCarg]=useState(true);
  useEffect(()=>{(async()=>{const{data}=await supabase.from('feedback').select('*,lider:lider_id(email,full_name),colaborador:colaborador_id(email,full_name)').eq('ciclo_id',cicloId).order('created_at',{ascending:false});setFbs(data||[]);setCarg(false);})();},[cicloId]);
  if(carg)return<p>Cargando...</p>;
...
function HistorialAdmin({ colaborador, onVolver }) {
  const [hist,setHist]=useState([]);const[carg,setCarg]=useState(true);
  useEffect(()=>{(async()=>{const{data}=await supabase.from('evaluaciones_historicas').select('*').eq('colaborador_id',colaborador.id).order('fecha_evaluacion',{ascending:false});setHist(data||[]);setCarg(false);})();},[]);
  if(carg)return<p>Cargando...</p>;
  return <div><button onClick={onVolver} style={{...s.btnInfo,marginBottom:16}}>← Volver</button><h3>📋 Historial: {colaborador.full_name||colaborador.email}</h3>{hist.length===0?<p style={{padding:40,color:'#94a3b8'}}>Sin historial.</p>:<table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr><th style={th}>Fecha</th><th style={th}>Rating</th></tr></thead><tbody>{hist.map(h=>(<tr key={h.id}><td style={td}>{new Date(h.fecha_evaluacion+'T12:00:00').toLocaleDateString('es-AR')}</td><td style={td}>{h.rating_final||'-'}</td></tr>))}</tbody></table>}</div>;
}

function EquipoLider({ cicloId, profile }) {
  const [equipo,setEquipo]=useState([]);const[colSel,setColSel]=useState(null);const[fbVis,setFbVis]=useState(null);
  useEffect(()=>{cargar();},[cicloId]);
  async function cargar(){const{data:{session}}=await supabase.auth.getSession();if(!session)return;const{data:d}=await supabase.from('profiles').select('id,email,full_name,area,seniority').eq('leader_id',session.user.id);if(d){const eq=await Promise.all(d.map(async(c)=>{const{data:ae}=await supabase.from('evaluaciones').select('id,estado,rating_promedio,comentarios_finales,puntuaciones(rating,competencia_id,comentario,competencias(nombre))').eq('colaborador_id',c.id).eq('tipo_evaluacion','autoevaluacion').eq('ciclo_id',cicloId).maybeSingle();const{data:le}=await supabase.from('evaluaciones').select('id,estado,rating_promedio').eq('colaborador_id',c.id).eq('tipo_evaluacion','evaluacion_lider').eq('ciclo_id',cicloId).maybeSingle();const{data:fb}=await supabase.from('feedback').select('*').eq('ciclo_id',cicloId).eq('colaborador_id',c.id).maybeSingle();return{...c,autoevaluacion:ae,evaluacionLider:le,feedback:fb};}));setEquipo(eq);}}
  ...
  return <div><h3>👥 Mi Equipo ({equipo.length})</h3>{equipo.length===0?<p>No tienes colaboradores.</p>:<div style={{display:'flex',flexDirection:'column',gap:12}}>{equipo.map(c=>(<div key={c.id} style={{...s.tarjetaStat,display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:12}}><div style={{flex:1}}><h4>{c.full_name||c.email}</h4><p style={{fontSize:13,color:'#64748b'}}>{c.area}·{c.seniority}</p><div style={{display:'flex',gap:16,marginTop:8,fontSize:12}}><span>📝 Auto: <strong style={{color:c.autoevaluacion?.estado==='enviado'?'#22c55e':'#f59e0b'}}>{c.autoevaluacion?.estado==='enviado'?'Enviada':'Pendiente'}</strong>{c.autoevaluacion?.rating_promedio&&<span style={{marginLeft:4}}>({c.autoevaluacion.rating_promedio})</span>}</span><span>👥 Mi eval: <strong style={{color:c.evaluacionLider?.estado==='enviado'?'#22c55e':c.evaluacionLider?'#f59e0b':'#94a3b8'}}>{c.evaluacionLider?.estado==='enviado'?'Completada':c.evaluacionLider?'Borrador':'Sin evaluar'}</strong></span><span>💬 FB: <strong style={{color:c.feedback?.confirmacion_colaborador?'#22c55e':c.feedback?'#f59e0b':'#94a3b8'}}>{c.feedback?.confirmacion_colaborador?'✅':c.feedback?'⏳':'-'}</strong></span></div></div><div style={{display:'flex',gap:8}}>{c.autoevaluacion?.estado==='enviado'&&<button onClick={()=>setColSel(c)} style={{...s.btnInfo,background:'#dbeafe',color:'#1e40af',fontWeight:600}}>👁️ Ver Auto</button>}<button onClick={()=>setFbVis(c)} style={{...s.btnInfo,background:'#fef3c7',color:'#92400e'}}>💬 FB</button><button onClick={()=>setColSel(c)} style={s.btnPrimario}>{c.evaluacionLider?'✏️ Editar':'📝 Evaluar'}</button></div></div>))}</div>}</div>;
}
...
function FeedbackForm({ feedback:col, cicloId, onVolver }) {
  ...
  async function guardar(){const{data:{session}}=await supabase.auth.getSession();await supabase.from('feedback').upsert({ciclo_id:cicloId,lider_id:session.user.id,colaborador_id:col.id,comentario_lider:com,fecha_feedback_lider:new Date()},{onConflict:'ciclo_id,colaborador_id'});alert('✅ Guardado');onVolver();}
  if(carg)return<p>Cargando...</p>;
...
function PanelColaborador({ userId, seniority, cicloId }) {
  ...
  async function enviar(){await guardar();const{data:ev}=await supabase.from('evaluaciones').select('id').eq('colaborador_id',userId).eq('tipo_evaluacion','autoevaluacion').eq('ciclo_id',cicloId).single();if(ev)await supabase.from('evaluaciones').update({estado:'enviado'}).eq('id',ev.id);setMsg('🎉 Enviada');setEvalData(p=>({...p,estado:'enviado'}));}
  ...
  return <div style={{maxWidth:900}}><h3>📝 Mi Autoevaluación</h3><p>Seniority: <strong>{seniority||'No definido'}</strong></p><p>Estado: <strong style={{color:enviada?'#231F20':'#f59e0b'}}>{enviada?'✅ Enviada':'📝 En progreso'}</strong></p>
    {feedback&&<div style={{padding:16,background:feedback.confirmacion_colaborador?'#dcfce7':'#fef3c7',borderRadius:10,marginBottom:20}}><h4>💬 Feedback de tu Líder</h4><p style={{color:'#475569',fontStyle:'italic'}}>{feedback.comentario_lider||'Sin comentarios.'}</p>{!feedback.confirmacion_colaborador?<button onClick={confirmarFB} style={{...s.btnPrimario,background:'#22c55e',marginTop:8,fontSize:13}}>✅ Confirmar Feedback</button>:<p style={{color:'#166534',marginTop:8}}>✅ Confirmado</p>}</div>}
...
const s = {
  ...
  tarjetaStat: { background: 'white', padding: 20, borderRadius: 12, marginBottom: 12, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' },
