      ...
      {competencias.map(comp => (
        <div key={comp.id} style={s.competenciaCard}>
          ...
          {autoevaluacion?.puntuaciones?.find(p => p.competencia_id === comp.id) && (
            <div style={{ padding: '8px 12px', background: '#fef3c7', borderRadius: 6, marginBottom: 8, fontSize: 13 }}>📝 Auto: <strong>{autoevaluacion.puntuaciones.find(p => p.competencia_id === comp.id).rating}</strong></div>
          )}
...
const sidebar = {
  ...
  subMenuItem: { padding: '10px 16px', borderRadius: 6, border: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 13, fontWeight: 400, transition: 'all 0.15s', width: '100%' },
