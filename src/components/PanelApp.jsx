...
function EvaluacionLider({ colaborador, cicloId, onVolver, soloLectura }) {
  ...
  useEffect(function() {
    async function cargarDatos() {
      try {
        var { data: comps } = await supabase.from('competencias').select('id, nombre, descripcion').eq('aplica_a', colaborador.seniority || 'Analista');
        setComp(comps || []);
        ...
        if (ae) {
          var { data: p } = await supabase.from('puntuaciones').select('id, rating, comentario, competencia_id, competencias(nombre)').eq('evaluacion_id', ae.id);
        ...
        if (liderEval) {
          ...
          var { data: nueva } = await supabase.from('evaluaciones').insert({ colaborador_id: colaborador.id, evaluador_id: session.user.id, tipo_evaluacion: 'evaluacion_lider', estado: 'borrador', ciclo_id: cicloId }).select().single();
  ...
  async function guardar() {
    ...
    if (!comFin?.trim()) { setMsg('❌ Comentarios finales obligatorios'); setTimeout(function() { setMsg(''); }, 4000); return; }
    var { data: ev } = await supabase.from('evaluaciones').select('id').eq('colaborador_id', colaborador.id).eq('tipo_evaluacion', 'evaluacion_lider').eq('ciclo_id', cicloId).single();
    if (!ev) return;
    ...
    for (var [cid, r] of Object.entries(ratings)) { await supabase.from('puntuaciones').upsert({ evaluacion_id: ev.id, competencia_id: cid, rating: r, comentario: comentarios[cid] || '' }, { onConflict: 'evaluacion_id, competencia_id' }); }
  ...
  async function enviar() {
    ...
    if (sinRating.length > 0) { setMsg('❌ Califica todas las competencias'); setTimeout(function() { setMsg(''); }, 4000); return; }
    await guardar();
    var { data: ev } = await supabase.from('evaluaciones').select('id').eq('colaborador_id', colaborador.id).eq('tipo_evaluacion', 'evaluacion_lider').eq('ciclo_id', cicloId).single();
    if (ev) await supabase.from('evaluaciones').update({ estado: 'enviado' }).eq('id', ev.id);
    setEvalData(function(p) { return { ...p, estado: 'enviado' }; });
    setMsg('🎉 Enviada'); setTimeout(function() { setMsg(''); }, 3000);
  }
  ...
  var prom = calcProm();
  var clasif = clasificarRating(prom ? parseFloat(prom) : 0);
  var autoRatings = {}; var autoComentarios = {};
  (autoEval?.puntuaciones || []).forEach(function(p) { autoRatings[p.competencia_id] = p.rating; autoComentarios[p.competencia_id] = p.comentario || ''; });
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      {competencias.map(function(comp) {
        ...
        return (
          <div key={comp.id} style={s.competenciaCard}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div><h5 style={{ margin: 0, color: '#231F20' }}>{comp.nombre}</h5><p style={{ fontSize: 13, color: '#64748b', margin: '4px 0' }}>{comp.descripcion}</p></div>
              <button onClick={function() { setShowInfo({ ...showInfo, [comp.id]: !showInfo[comp.id] }); }} style={s.btnInfo}>{showInfo[comp.id] ? '🔼 Ocultar info' : '🔽 Ver info'}</button>
            </div>
            {autoEval?.estado === 'enviado' && (
              <div style={{ marginBottom: 12, padding: 12, background: '#D4D2C6', borderRadius: 8, border: '1px solid #cbd5e1' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#231F20' }}>📝 Autoevaluacion: <span style={{ fontSize: 16, fontWeight: 700 }}>{autoRating || '-'}</span></span>
                  {gap !== null && <span style={{ fontSize: 12, fontWeight: 600, color: gap > 0 ? '#22c55e' : gap < 0 ? '#dc2626' : '#64748b' }}>{gap > 0 ? '↑' : gap < 0 ? '↓' : '='} Tu eval vs Auto: {gap > 0 ? '+' : ''}{gap}</span>}
                </div>
                {autoComentario && <div style={{ marginTop: 6, padding: '6px 8px', background: 'white', borderRadius: 4, fontSize: 12, color: '#475569', fontStyle: 'italic' }}>"{autoComentario}"</div>}
              </div>
            )}
            <div style={s.ratingRow}>
              <span style={{ fontSize: 12, color: '#64748b', marginRight: 4 }}>Tu eval:</span>
              {[1, 2, 3, 4, 5].map(function(r) {
                return <button key={r} onClick={function() { if (!enviada && !soloLectura) setRatings({ ...ratings, [comp.id]: r }); }} style={{ ...s.ratingBtn, backgroundColor: ratings[comp.id] === r ? '#231F20' : '#f1f5f9', color: ratings[comp.id] === r ? 'white' : '#475569', border: ratings[comp.id] === r ? '2px solid #231F20' : '2px solid #e2e8f0', cursor: enviada || soloLectura ? 'not-allowed' : 'pointer', opacity: enviada || soloLectura ? 0.7 : 1 }} disabled={enviada || soloLectura}>{r}</button>;
              })}
              {ratings[comp.id] && <span style={{ marginLeft: 8, fontSize: 14, fontWeight: 600, color: '#231F20' }}>Nivel {ratings[comp.id]}</span>}
            </div>
            {showInfo[comp.id] && (
              <div style={s.ratingInfoBox}>
                {[1, 2, 3, 4, 5].map(function(r) { return <div key={r} style={{ ...s.ratingInfoItem, borderLeft: '4px solid ' + (ratings[comp.id] === r ? '#231F20' : '#e2e8f0'), backgroundColor: ratings[comp.id] === r ? '#f8fafc' : 'white', fontWeight: ratings[comp.id] === r ? '600' : '400' }}><strong>Nivel {r}:</strong> <RatingDesc competenciaId={comp.id} rating={r} /></div>; })}
              </div>
            )}
            <textarea value={comentarios[comp.id] || ''} onChange={function(e) { if (!enviada && !soloLectura) setComent({ ...comentarios, [comp.id]: e.target.value }); }} placeholder="Tu comentario como líder (obligatorio)" style={{ ...s.textareaSmall, borderColor: enviada || soloLectura ? '#D4D2C6' : (comentarios[comp.id]?.trim() ? '#D4D2C6' : '#dc2626'), backgroundColor: enviada || soloLectura ? '#f8fafc' : 'white', cursor: enviada || soloLectura ? 'not-allowed' : 'text' }} readOnly={enviada || soloLectura} />
          </div>
        );
      })}
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {enviada || soloLectura ? <p style={{ color: '#475569', padding: 12, background: '#f8fafc', borderRadius: 8 }}>{comFin || 'Sin comentarios.'}</p> : <textarea value={comFin} onInput={function(e) { setComFin(e.target.value); }} style={{ ...s.textarea, borderColor: comFin?.trim() ? '#D4D2C6' : '#dc2626' }} placeholder="Escribe tus comentarios finales..." />}
      </div>

      {prom && <div style={{ marginTop: 24, padding: 20, background: 'white', borderRadius: 12, border: '2px solid ' + clasif.color, textAlign: 'center' }}><p style={{ color: '#64748b', margin: 0, fontSize: 14 }}>Resultado Final</p><p style={{ fontSize: 48, fontWeight: 700, color: clasif.color, margin: '8px 0' }}>{prom}</p><p style={{ fontSize: 18, fontWeight: 600, color: clasif.color, margin: 0 }}>{clasif.texto}</p></div>}
      {msg && <div style={s.mensajeToast}>{msg}</div>}
      {!enviada && !soloLectura && <div style={{ display: 'flex', gap: 12, marginTop: 20, marginBottom: 40 }}><button onClick={guardar} style={s.btnSecundario}>💾 Guardar Borrador</button><button onClick={enviar} style={s.btnPrimario}>📤 Enviar Evaluacion</button></div>}
      {enviada && <div style={{ ...s.bannerEnviado, marginTop: 20 }}>✅ Tu evaluacion como lider ha sido enviada y no puede modificarse.</div>}
    </div>
  );
}
